...
def get_filter_metrics(plot_ratios=True):
    t_pairs = []
    f_pairs = []
    purity = []
    efficiency = []
...
def get_emb_metrics(hits, truth, emb_model, adjacent=True, radius=0.4):
    purity = []
    efficiency = []
    neighbors = get_emb_neighbors(hits[feature_names].values, emb_model, radius)
    n_nbr = len(neighbors)
    for nbr in tqdm(range(n_nbr)):
        hood = neighbors[nbr]
        vid = hits['volume_id'].values
        lid = hits['layer_id'].values
        #if seed hit is noise it has no true neighbors
        truth_np = np.array(truth.values)
        pid = truth_np[nbr]
        seed_hit = hits.iloc[nbr]
        t_nbrs = []
        if pid!=0:
            track = truth[truth['particle_id']==pid]['hit_id']
            track_ids = hits[hits['hit_id'].isin(track)]
            #remove duplicate
            true_hits = track_ids[track_ids['hit_id'] != seed_hit['hit_id']].index.values.astype(int)
            if adjacent:
                t_nbrs = filter_one_neighborhood(seed_hit['volume_id'],seed_hit['layer_id'],true_hits,vid,lid) 
        if adjacent:
            hood = filter_one_neighborhood(seed_hit['volume_id'],seed_hit['layer_id'],hood,vid,lid) 
        #true_in_hood = sum(map(lambda n : n in t_nbrs, hood))
        true_in_hood = len(list(set(t_nbrs).intersection(hood)))
        p = 0
        e = 0
        if len(hood) >0:
            p = true_in_hood / len(hood)
        if len(t_nbrs) >0:
            e = true_in_hood / len(t_nbrs)
        purity.append(p)
        efficiency.append(e)
    
    return statistics.mean(purity), statistics.mean(efficiency)
    
    
def get_noise_emb_metrics(hits, truth, emb_model, event_name, noise_keeps, adjacent=True, radius=0.4):
    purity = []
    efficiency = []
    for noise_keep in noise_keeps:
        hits, truth, emb_model, filter_model = get_data(event_name, None, noise_keep)
        p, e = get_emb_metrics(hits, truth, emb_model, True, radius)
        purity.append(p)
        efficiency.append(e)
